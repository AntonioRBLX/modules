...
function module:Compute(startPosition,targetCharacter,projectileSpeed,projectileGravity,argumentTable)
	...
	local asynchronous = if argumentTable.Asynchronous == nil then false else argumentTable.Asynchronous
	...
	assert(typeof(asynchronous) == "boolean","Asynchronous must be a boolean")
	...
	local x
	local y
	local floorHit = Instance.new("Part")
	...
	local wallHit = Instance.new("Part")
	...
	local ceilHit = Instance.new("Part")
	...
	local function checkHighestLowest(obj,typ)
		...
		local blockcast = workspace:Blockcast(CFrame.new(Vector3.new(simulatedPos.X,simulatedPos.Y + dirY / math.abs(dirY),simulatedPos.Z)) * CFrame.Angles(0,math.rad(targetRoot.Orientation.Y),0),Vector3.new(2,0,1),Vector3.new(0,dirY,0),params)
	...
	--end
	local spawn = tick()
	local computepathcoroutine = coroutine.create(function()
		while true do
			...
			local checkFloorIntercept = workspace:Blockcast(CFrame.new(prevSimulatedPos - Vector3.yAxis) * CFrame.Angles(0,math.rad(targetRoot.Orientation.Y),0),Vector3.new(2,0,1),(prevSimulatedPos - Vector3.yAxis) - (simulatedPos - Vector3.new(0,1.00001,0)),raycastParams)
			local checkCeilIntercept = workspace:Blockcast(CFrame.new(prevSimulatedPos + Vector3.yAxis) * CFrame.Angles(0,math.rad(targetRoot.Orientation.Y),0),Vector3.new(2,0,1),(prevSimulatedPos + Vector3.yAxis) - (simulatedPos + Vector3.new(0,1.00001,0)),raycastParams)
			...
			if asynchronous and spawn + 1/60 < tick() then
				coroutine.yield()
			end
		end
	end)
